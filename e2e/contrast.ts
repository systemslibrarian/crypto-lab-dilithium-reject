 ...
 *    `color-mix()`. Essentially every string on this page is one: `body` paints
 *    `linear-gradient(180deg, var(--bg-1), var(--bg-2))` with a cyan and a pink
 *    `radial-gradient` halo washed over it, every `.card` lays translucent
 *    `--panel` on that, and the surfaces the readouts actually sit on —
 *    `.iteration.accepted` / `.iteration.rejected`, `.check.fail`,
 *    `.status-accepted` / `.status-rejected`, `.check.pass .check-pill`,
 *    `.sim-note`, `.cl-hero-why` — are each a
 *    `color-mix(… var(--accept|--reject|--gold|--cyan) N%, transparent)` on top
 *    of that again. axe files all of them under `incomplete`.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Here `.cl-hero-sub` is at
 *    `.85`, and every control this lab disables while the worker runs is at
 *    `.5` (that half is then cleared honestly, by the inactive-component
 *    exemption below, rather than by injection). The gate this replaces
 *    neutralised motion with an injected `!important` style tag; the same
 *    reflex applied to opacity fabricates contrast results outright.
 *
 ...
 *    boxes are `.iteration-feed` (a `role="log"` scroller capped at 520px that
 *    a long rejection run overflows), the two `.table-wrap`s around the scheme
 *    comparison and the symbol glossary, and `.tour-panel`. Content scrolled
 *    past them is not dimmed or partly drawn — it is absent from the frame, and
 ...
 *    load-bearing here, not defensive: all three charts are hand-built SVG that
 *    label themselves in `<text>` — the percentage and iteration axis ticks
 *    (`.axis-label`), the two rotated `.axis-title`s, the `mean ≈ 5.1` and
 *    `median` tags, and the `KS = 0.0xx` gap label. Every one of those `<text>`
 *    nodes is emitted AFTER the `.grid` rules, the `.axis` lines and the bars,
 *    so an ancestor-only walk would composite them onto the `<svg>`'s own
 *    transparent background instead of onto what is drawn beneath, and report a
 ...
 * uses. Nothing here needs narrowing: this lab's transient states (the batch
 * progress line, the "Hunting for a real k-iteration trace…" message, the
 * disabled-control wash while the worker runs) reuse `.summary`, `.meta` and
 * `button[disabled]` — classes that are on the page in the settled states the
 * gate already scans — so there is no styling that exists only for a few
 * hundred milliseconds. The parameter is kept because the narrowed form is what
 * makes a short-lived state measurable at all: a class that lives 500ms is
 * shorter than a full axe pass, so a whole-page scan would catch it only
 * sometimes, and a sometimes-measured assertion is worse than none. Scoped to
 * one subtree the walk costs a couple of milliseconds, so a caller can set the
 * state, measure, and confirm the state is still set.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven scan here walks an iteration feed of up to twenty cards each
     * carrying four `.check` rows, a twelve-cell stats grid, a forty-bar
     * histogram whose every bar is its own `<g>` with a `<title>`, a second
     * histogram of measured signing times, a CDF overlay, and two tables — and
     ...
     * and rects thousands of times, and the gate scans two dozen states per
     ...
     * that is `.iteration-feed`, the two `.table-wrap`s and `.tour-panel`.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. Both matter here: every chart draws its `.grid` rules
      // and its two `.axis` lines as stroke-only `<line>`s spanning the whole
      // plot, and the PMF and CDF curves are `<polyline>`/`<path>` with an
      // explicit `fill: none`, which `resolve` returns null for — so they are
      // skipped by a second route as well.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `checkVisibility()` is the only test that catches content whose
      // geometry survives but whose paint does not. This lab has no <details>;
      // it hides regions with the `hidden` ATTRIBUTE instead — `#tamper-panel`
      // until a signature exists, `.seed-field` and `.det-note` until
      // reproducible mode is on, `#leak-game` until the broken scenario has
      // run. `hidden` resolves to `display: none`, which the check above
      // already catches, but a `content-visibility` or `visibility` route added
      // later would not be, and the previous gate's answer to all of this was
      // to strip `hidden` from every element on the page at once and scan a
      // document no visitor can load. The gate now reaches each one by working
      // the control that reveals it.
      ...
      // page has two of them: the shared header's `.cl-skip-link` parks at
      // `top:-3rem` and the lab's own `.skip-link` at `top:-40px`, each sliding
      // in only on focus. Measuring a parked copy invents a failure for text
      // that is not on screen; the focused rendering is a real state and the
      // gate scans both of them explicitly instead.
     ...
     * here may hide a real value behind that attribute. This lab has exactly
     * three `aria-hidden` subtrees that own characters, and all three were
     * measured by hand rather than assumed:
     *
     *  - the shared header's hamburger and GitHub `<svg>`s — glyph-free;
     *  - `.secret`, the row of `█` blocks standing in for the secret key. It
     *    is a redaction, not a value; `--reject` sampled from real pixels
     *    against the panel behind it is 4.92:1 in dark and 7.55:1 in light, so
     *    it would pass anyway;
     *  - `.chart-legend`, the colour key under each chart. It is hidden because
     *    "the cyan swatch is the observed bars" is information a screen reader
     *    cannot use; the same data reaches the accessibility tree through each
     *    chart's `aria-label` and, for the histogram, through the `.sr-only`
     *    data table. Its ink is `--muted` on `--panel`, the same pair the
     *    measured `.meta` paragraphs beside it use — 7.67:1 dark, 8.85:1 light,
     ...
     *
     * This page is full of the case. Every bar in the iteration histogram is a
     * `<g><title>…</title><rect/></g>`; the timing bars and the PMF dots carry
     * `<title>`s too. `<title>` is character data SVG never paints — it is the
     * tooltip and the accessible name — and reporting a ratio for it would be
     * reporting on ink that was never laid down. (A `<title>` also lays out to a
     * zero-sized box, so `isVisible` reaches most of them first; the two guards
     * agree, and this one is the reason rather than the accident.) It also
     * covers the way this page could grow the mistake: all three charts are
     * assembled by interpolating into template literals, and interpolating a
     * string ARRAY into one (`<g>${dots}</g>`) makes JS join it with commas,
     * leaving a run of "," text nodes inside the group.
     */
    ...
    for (const el of Array.from(document.querySelectorAll(rootSelector))) {
      ...
      while (node) {
        ...
        const paint =
          node === el || intersects(textBox, rectOf(node)) ? paintAt(ncs, rectOf(node), point) : TRANSPARENT;
...
export function formatContrastFailures(failures: ContrastFailure[]): string[] {
  return failures.map(
    (f) => `${f.ratio}:1 (needs ${f.required}:1) ${f.selector} — fg ${f.foreground} on ${f.background} — "${f.text}"`,
  );
}
